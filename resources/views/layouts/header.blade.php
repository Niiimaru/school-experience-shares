<!-- header-->
<header class="l-header" id="header">
  <div class="p-header-logo">セブ島 エンジニア留学なら シードテックスクール<img class="p-header-logo__img" src="assets/images/common/logo.svg" alt="シードテックスクール" width="142" heigth=""></div><span class="c-main-nav__btn" aria-label="MEMU" id="headerMenu"></span>
  <nav class="c-main-nav">
  <div class="c-main-nav__container">
      <ul class="c-main-nav__parent">
      <li class="is-currentt"><a href="{{url('/')}}">ホーム</a></li>
      </ul>
      <div class="c-main-nav__cv-btns"><a class="c-main-nav__cv-btns__request" href="{{url('/document')}}">資料請求</a><a class="c-main-nav__cv-btns__reserve" href="{{url('/counseling')}}">面談のご予約</a></div>
  </div>
  </nav>
</header>
<!-- /header-->